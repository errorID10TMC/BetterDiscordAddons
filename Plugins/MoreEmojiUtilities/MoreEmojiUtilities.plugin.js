...
const { PrefixSecurityEnum } = require("tough-cookie");
  ...
  const config = {
    info: {
      ...
      authors: [
        {
          name: "errorID10T",
          discord_id: "562378063820750858",
          github_username: "errorID10TMC",
        },
      ],
      version: "0.1.0",
      description: "Allows you to blacklist, favorite and send emojis as a link.",
      github:
    ...
    changelog: [
      {
        ...
        type: "added",
        items: ["Added `Send as Image` option in the emoji picker."],
      },
    ],
    main: "index.js",
  };
    ...
    : (([Plugin, Api]) => {
        const plugin = (Plugin, Api) => {
          ...
          return class EmojiUtilities extends Plugin {
            onStart() {
              this.disabledEmojis = PluginUtilities.loadData(
                this.getName(),
                "disabledEmojis",
                []
              );
              this.favoriteEmojis = PluginUtilities.loadData(
                this.getName(),
                "favoriteEmojis",
                []
              );
              this.disabledEmojis = this.disabledEmojis.filter(
                (e) => typeof e === "string"
              );
              this.favoriteEmojis = this.favoriteEmojis.filter(
                (e) => typeof e === "string"
              );
              ...
              Utilities.suppressErrors(
                this.patchEmojiComponent.bind(this),
                "Emoji Patch"
              )(this.promises);
              Utilities.suppressErrors(
                this.patchMessageContextMenu.bind(this),
                "Message Context Menu Patch"
              )();
              Utilities.suppressErrors(
                this.patchEmojiPicker.bind(this),
                "Emoji Picker Patch"
              )(this.promises);
              Utilities.suppressErrors(
                this.patchReactions.bind(this),
                "Reactions Patch"
              )();
            }
            ...
            addCategories() {
              ...
              Object.defineProperty(EmojiUtils, "categories", {
                writable: true,
                value: [
                  "favorite emojis",
                  ...EmojiUtils.originalCategories,
                  "blacklist",
                ],
              });
              ...
              Patcher.after(EmojiStore, "getByCategory", (_, args) => {
                if (args[0] == "favorite emojis")
                  return this.favoriteEmojis
                    .map((e) => this.resolveEmoji(e))
                    .filter((e) => this.isResolvable(e));
                if (args[0] == "blacklist")
                  return this.disabledEmojis
                    .map((e) => this.resolveEmoji(e))
                    .filter((e) => this.isResolvable(e));
              });

              // Give the fake categories an icon
              const EmojiCategoryIcon = WebpackModules.getModule(
                (m) =>
                  m.default &&
                  m.default.type &&
                  m.default.type.toString().includes("FOOD")
              );
              Patcher.after(EmojiCategoryIcon.default, "type", (_, [props]) => {
                if (props.categoryId == "favorite emojis")
                  return DiscordModules.React.createElement(
                    FavoriteIcon,
                    props
                  );
                if (props.categoryId == "blacklist")
                  return DiscordModules.React.createElement(
                    DisabledIcon,
                    props
                  );
            ...
            addBlacklistFilter() {
              ...
              Patcher.after(
                ...
                (thisObject, methodArguments, returnValue) => {
                  ...
                  if (
                    emoji.uniqueName &&
                    this.disabledEmojis.includes(emoji.uniqueName)
                  )
                    return true;
                  if (emoji.id && this.disabledEmojis.includes(emoji.id))
                    return true;
                  return returnValue;
                }
              );
              ...
              Patcher.after(
                ...
                (t, a, returnValue) => {
                  ...
                  returnValue.getFrequentlyUsedEmojis = function () {
                    ...
                    return originalReturn.filter(
                      (e) => !plugin.isBlacklisted(e.id || e.uniqueName)
                    );
                  };
                }
              );
              ...
              Patcher.after(
                ...
                "getByCategory",
                (_, args, returnValue) => {
                  if (args[0] == "favorite emojis" || args[0] == "blacklist")
                    return;
                  return returnValue.filter(
                    (e) => !this.isBlacklisted(e.uniqueName)
                  );
                }
              );
            }
            ...
            patchReactions() {
              const ReactionsComponent = WebpackModules.getByDisplayName(
                "Reactions"
              );
              Patcher.after(
                ReactionsComponent.prototype,
                "render",
                (thisObject, methodArguments, returnValue) => {
                  if (
                    !returnValue ||
                  ...
                  for (let r = 0; r < reactions.length; r++) {
                    ...
                    const emoji = this.resolveEmojiIdentifier(
                      reactions[r].props.emoji.id ||
                        reactions[r].props.emoji.name
                    );
            ...
            async patchEmojiComponent(promiseState) {
              const Emoji = await ReactComponents.getComponentByName(
                "Emoji",
                ".emoji"
              );
              if (promiseState.cancelled) return;
              Patcher.after(
                Emoji.component.prototype,
                "render",
                (thisObject, methodArguments, returnValue) => {
                  const emoji = this.resolveEmojiIdentifier(
                    thisObject.props.emojiId || thisObject.props.emojiName
                  );
            ...
            patchMessageContextMenu() {
              const MessageContextMenu = WebpackModules.getModule(
                (m) =>
                  m.default && m.default.displayName == "MessageContextMenu"
              );
              ...
              Patcher.after(
                ...
                (_, [props], retVal) => {
                  ...
                  const actions = [
                    {
                      label: "Blacklist Emoji",
                      closeOnClick: true,
                      action: this.currentEmojiContext.blacklist,
                    },
                    {
                      label: this.currentEmojiContext.isFavorite
                        ? "Remove Favorite"
                        : "Favorite Emoji",
                      closeOnClick: true,
                      action: this.currentEmojiContext.toggleFavorite,
                    },
                  ];

                  const original = retVal.props.children;
                  const newOne = DCM.buildMenuChildren([
                    { type: "group", items: actions },
                  ]);
                  if (Array.isArray(original)) original.splice(1, 0, newOne);
                  else
                    retVal.props.children[0].props.children = [
                      original,
                      newOne,
                    ];
                }
            ...
            async patchEmojiPicker() {
              const EmojiPickerListRow = WebpackModules.getModule(
                (m) =>
                  m.default && m.default.displayName == "EmojiPickerListRow"
              );
              Patcher.after(
                EmojiPickerListRow,
                "default",
                (thisObject, args, returnValue) => {
                  ...
                  for (let e = 0; e < emojiComponents.length; e++) {
                    const emojiObj =
                      emojiComponents[e].props.children.props.emoji;
                    const emoji = this.resolveEmojiIdentifier(
                      emojiObj.id || emojiObj.allNamesString
                    );
                    emojiComponents[e].props.onContextMenu = (event) => {
                      ...
                      // If non default emoji add "Send as Image"
                      if (emoji.match(emojiIdRegex)) {
                        items.push({
                          label: "Send as Image",
                          closeOnClick: true,
                          action: () => {
            ...
            sendAsImage(value, name, animated) {
              const emoji = this.resolveEmojiIdentifier(value);
              if (emoji.match(emojiIdRegex)) {
                const channelId = document.location.href.split("/")[
                ...
                // If the emoji name has an underscore in it the name will default to "image" when downloaded from Images.weserv.nl
                if (animated) {
                  messageContent =
                    "https://images.weserv.nl/emoji.gif?url=https%3A%2F%2Fapi.allorigins.win%2Fraw%3Furl%3D" +
                    encodeURIComponent(encodeURIComponent(emojiUrl)) +
                    "&w=48&h=48&fit=contain&filename=" +
                    name +
                    "&n=-1";
                } else {
                  messageContent =
                ...
                // Runs if emoji value was not an emoji id. This should not happen now since the send option is disabled for default emojis
              }
            }
            ...
            resolveEmoji(value) {
              ...
              if (typeof value === "object")
                return !value.url && !value.allNamesString ? null : value; // if given an old emote, return null || if given emote, return emote

              const idMatch = value.match(emojiIdRegex); // Check if it's an ID first
              if (idMatch && idMatch.length == 2)
                return this.findEmoji(idMatch[1]);
              ...
              const matched = value.match(
                DiscordModules.EmojiStore.EMOJI_NAME_RE
              ); // Grab name if it has colons or diversity
              if (matched && matched.length == 2)
                return EmojiStore.getByName(matched[1]);
            ...
            resolveEmojiIdentifier(value) {
              ...
              if (matched && matched.length == 2)
                return EmojiStore.getByName(matched[1]).uniqueName;
            ...
            findEmoji(id) {
              return Object.values(EmojiUtils.getGuilds())
                .map((m) => m.emojis)
                .flat()
                .find((e) => e.id == id);
            }
